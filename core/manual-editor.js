...
export class ManualEditor {
    ...
    saveEdits() {
        ...
        // Apply auto-closure BEFORE saving (force close mode)
        this.editedCoordinates = this.ensureAutoClose(this.editedCoordinates, true);
        
        // Return edited coordinates
        const saved = JSON.parse(JSON.stringify(this.editedCoordinates));
        
        this.addChangeLog('Changes saved (auto-closed if needed)');
     ...
     * @param {boolean} forceClose - If true, always close regardless of gap size
     * @returns {Array} - Coordinates with auto-closure applied if needed
     */
    ensureAutoClose(coords, forceClose = false) {
        ...
        const gap = this.calculateHaversineDistance(first, last);
        
        // MODE 1: Force close (manual editing) - ALWAYS close
        if (forceClose) {
            console.log(`✅ Force closing polygon (gap: ${gap.toFixed(2)}m)`);
            return [...coords, [first[0], first[1]]];
        }
        
        // MODE 2: Auto-close only if gap ≤ 0.5m (validation)
        if (gap > 0 && gap <= 0.5) {
            console.log(`✅ Auto-closing (gap: ${gap.toFixed(3)}m)`);
            return [...coords, [first[0], first[1]]];
        }
        
        if (gap > 0.5) {
            console.log(`⚠️ Gap: ${gap.toFixed(2)}m - not auto-closing`);
        }
