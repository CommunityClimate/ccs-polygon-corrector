...
export class MapManager {
    ...
    initialize() {
        ...
        try {
            ...
            // Don't add default tile layer here - switchBaseLayer will handle it
            // This prevents the layer from being immediately removed when switching views
            
            // Initialize layer groups
            this.initializeLayerGroups();
            
            // Set initial base layer via switchBaseLayer (ensures it stays)
            this.switchBaseLayer('osm');
            
            console.log('✅ Map initialized successfully at', APP_CONFIG.MAP.DEFAULT_CENTER);
            console.log('✅ Layer groups created');
            console.log('✅ Default OSM layer set');
    ...
    initializeLayerGroups() {
        ...
        // Base layers with error handling and resource management
        this.baseLayers = {
            ...
            satellite: L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', {
                ...
                console.log('✅ Satellite tile loaded:', e.tile.src);
            }),
    ...
    drawCorrected(coordinates, options = {}) {
        ...
        // Determine color based on correction method
        let color = APP_CONFIG.COLORS.CORRECTED; // Default green
        let correctionLabel = 'Corrected';
        
        if (options.field?.correction?.method) {
            if (options.field.correction.method === 'auto_correct') {
                color = '#ffc107'; // Yellow/Orange for auto-corrected
                correctionLabel = 'Auto-Corrected';
            } else if (options.field.correction.method === 'manual_edit') {
                color = '#667eea'; // Blue/Purple for manually-corrected
                correctionLabel = 'Manually-Corrected';
            }
        }
        
        // Allow color override from options
        ...
        if (options.field) {
            ...
            console.log(`✅ Popup bound to ${correctionLabel.toLowerCase()} polygon`);
        } else if (options.popup) {
            polygon.bindPopup(options.popup);
        }
        ...
        fields.forEach((field, index) => {
            ...
            try {
                ...
                if (field.correctedCoordinates && field.correctedCoordinates.length >= 3) {
                    // Determine color based on correction method
                    let correctionColor = '#28a745'; // Default green
                    let correctionLabel = 'Corrected';
                    
                    if (field.correction?.method === 'auto_correct') {
                        correctionColor = '#ffc107'; // Yellow/Orange for auto-corrected
                        correctionLabel = 'Auto-Corrected';
                    } else if (field.correction?.method === 'manual_edit') {
                        correctionColor = '#667eea'; // Blue/Purple for manually-corrected
                        correctionLabel = 'Manually-Corrected';
                    }
