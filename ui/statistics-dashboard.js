...
export class StatisticsDashboard {
    ...
    static calculateStatistics() {
        ...
        const stats = {
            ...
            invalid: 0,
            canBeFixed: 0,
            needsManual: 0,  // CHANGED: Will use manual flags as source of truth
            duplicates: 0,
            parseFailed: 0,
            notValidatedYet: 0,
            autoCorrected: 0,
        ...
        allFields.forEach(field => {
            ...
            // FIXED: Accept correction even if correctedCoordinates is missing
            // (some corrections might not save coordinates but still mark as applied)
            const hasAppliedCorrection = field.correction?.applied === true;
            const hasCorrectedCoords = field.correctedCoordinates && 
                                      Array.isArray(field.correctedCoordinates) && 
                                      field.correctedCoordinates.length > 0;
            
            // Count as corrected if EITHER:
            // - correction.applied is true AND correctedCoordinates exist, OR
            // - correction.applied is true AND validation now passes
            const isCorrected = hasAppliedCorrection && 
                              (hasCorrectedCoords || validation.verra?.overallStatus === 'PASS');
            ...
            // ADDITIONAL CHECK: If correction.applied but no coordinates and still failing validation,
            // this might be an incomplete correction - let it fall through to normal validation
            if (hasAppliedCorrection && !hasCorrectedCoords && validation.verra?.overallStatus !== 'PASS') {
                console.warn(`⚠️ Field ${field.ccsFieldId} has correction.applied but no coordinates and still failing validation - might be incomplete correction`);
            }
        ...
        // ═══════════════════════════════════════════════════════════════
        // CRITICAL FIX: Calculate "Needs Manual" based on MANUAL FLAGS
        // Use expert human review as source of truth, not system validation
        // ═══════════════════════════════════════════════════════════════
        
        // Get fields with manual flags (expert-identified real issues)
        const manuallyFlaggedFields = allFields.filter(f => {
            if (!f.manualFlags) return false;
            return Object.values(f.manualFlags).some(v => v === true);
        });
        
        // Get fields that have been corrected
        const correctedFieldIds = new Set(
            allFields.filter(f => f.correction?.applied === true)
                     .map(f => f.ccsFieldId)
        );
        
        // TRUE "Needs Manual" = Flagged but NOT yet corrected
        const trueNeedsManual = manuallyFlaggedFields.filter(f => 
            !correctedFieldIds.has(f.ccsFieldId)
        );
        
        // Override the system's validation count with our expert count
        stats.needsManual = trueNeedsManual.length;
        
        console.log('');
        console.log('═══════════════════════════════════════════════════════════════');
        console.log('✅ USING MANUAL FLAGS AS SOURCE OF TRUTH:');
        console.log('═══════════════════════════════════════════════════════════════');
        console.log(`   Total manually flagged (expert review): ${manuallyFlaggedFields.length}`);
        console.log(`   Already corrected: ${manuallyFlaggedFields.length - trueNeedsManual.length}`);
        console.log(`   TRUE remaining work: ${trueNeedsManual.length}`);
        console.log(`   Progress: ${Math.round(((manuallyFlaggedFields.length - trueNeedsManual.length) / manuallyFlaggedFields.length) * 100)}% complete`);
        console.log('═══════════════════════════════════════════════════════════════');
        console.log('');
    ...
    static displayStatistics(containerId = 'statisticsDashboard') {
        ...
        // Update VERRA READY count
        // CHANGED: Total fields - TRUE needs manual = Verra Ready
        // This now reflects expert human review, not system validation
        ...
        const verraReadyCount = stats.total - stats.needsManual - stats.duplicates - stats.parseFailed;
        ...
        // NEW: Show/hide MANUAL FLAGS box
