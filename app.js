...
• New fields (imported fresh): ${result.newFields || 0}`;
    ...
    loadField(field) {
        ...
        // ── NEIGHBOUR OVERLAY — passed from CCS Field Tree Catalog ─────────
        // Draw overlapping (purple) and adjacent (amber) neighbours from localStorage
        this._drawNeighbourOverlays(field.ccsFieldId || field.fieldId);
    ...
    /**
     * Draw neighbour field polygons passed from the CCS Field Tree Catalog
     * via localStorage. Overlapping neighbours shown in purple, adjacent in amber.
     */
    _drawNeighbourOverlays(fieldId) {
      // Clear any existing neighbour layers
      if (this._neighbourLayers) {
        this._neighbourLayers.forEach(l => {
          try { this.mapManager.map.removeLayer(l); } catch(e) {}
        });
      }
      this._neighbourLayers = [];

      try {
        const raw = localStorage.getItem('ccs_corrector_neighbours');
        if (!raw) return;
        const data = JSON.parse(raw);

        // Only use if data is for this field and less than 5 minutes old
        if (data.fieldId !== fieldId) return;
        if (Date.now() - data.timestamp > 5 * 60 * 1000) return;
        if (!data.neighbours || !data.neighbours.length) return;

        const leafletMap = this.mapManager.map;
        if (!leafletMap) return;

        data.neighbours.forEach(n => {
          try {
            const color   = n.type === 'overlap' ? '#a78bfa' : '#f59e0b';
            const label   = n.type === 'overlap' ? '⧖ Overlapping' : '◻ Adjacent';
            const layer = L.geoJSON(n.geoJSON, {
              style: {
                color,
                weight:      n.type === 'overlap' ? 2.5 : 1.5,
                opacity:     0.9,
                fillColor:   color,
                fillOpacity: n.type === 'overlap' ? 0.08 : 0.04,
                dashArray:   n.type === 'adjacent' ? '5 5' : null
              }
            }).bindTooltip(
              `<strong>${label}</strong><br>${n.fieldId}<br>${n.owner}`,
              { sticky: true, className: 'neighbour-tooltip' }
            ).addTo(leafletMap);

            this._neighbourLayers.push(layer);
          } catch(e) {
            console.warn('Could not draw neighbour:', n.fieldId, e);
          }
        });

        if (this._neighbourLayers.length > 0) {
          console.log(`✓ Drew ${this._neighbourLayers.length} neighbour overlay(s) from CCS Field Tree Catalog`);
          this.uiManager?.showToast(
            `${this._neighbourLayers.length} neighbour field(s) shown — purple: overlapping · amber: adjacent`,
            'info'
          );
        }
      } catch(e) {
        console.warn('Could not load neighbour overlays:', e);
      }
    }
