...
• Successfully imported: ${result.success || result.count} field(s) with valid polygons
• Final count in system: ${actualCount} field(s)`;
                ...
                if (failedStats.total > 0) {
                    ...
                    summaryMessage += `\n\n💡 View failed records using "Parse Failed" filter`;
                }

                if (result.empty && result.empty > 0) {
                    summaryMessage += `\n• Empty GeoJSON: ${result.empty} row(s) (included in failures above)`;
                }

                summaryMessage += `\n\n${result.success && (result.success > actualCount) ? 
                  `⚠️ Note: ${result.success - actualCount} duplicate field IDs were removed` : 
                  (result.success && result.success < actualCount ? 
                    `⚠️ Note: ${actualCount - result.success} duplicate field IDs were kept` : '')}`;
