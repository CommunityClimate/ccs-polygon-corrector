...
export class StorageService {
    ...
    // PERFORMANCE MODE: Memory-only for large datasets
    // For 22k+ fields, localStorage quota (5-10 MB) is insufficient
    // Memory-only mode handles 100+ MB datasets without quota errors
    // WORKFLOW: Import → Work → Export regularly to save progress
    static memoryOnlyMode = true; // Keep TRUE for datasets > 5,000 fields
    ...
    static bulkSaveFields(newFields) {
        ...
        if (this.memoryOnlyMode) {
            // FIXED: Use Map to properly handle duplicates (replace, not append)
            const fieldMap = new Map();
            
            // Add existing fields to map
            this.inMemoryFields.forEach(f => {
                fieldMap.set(f.ccsFieldId, f);
            });
            ...
            // Add or update with new fields (this replaces duplicates!)
            newFields.forEach(field => {
                ...
                fieldMap.set(field.ccsFieldId, field);  // Replace duplicate!
            });
            
            // Replace in-memory array with deduplicated data
            this.inMemoryFields = Array.from(fieldMap.values());
            
            console.log(`✅ Bulk saved ${newFields.length} fields in MEMORY`);
            console.log(`   - New fields: ${newCount}`);
            console.log(`   - Updated existing: ${updatedCount}`);
            console.log(`   - Total in memory: ${this.inMemoryFields.length}`);
    ...
    // NEW: Store import statistics (duplicates found, etc.)
    static saveImportStats(stats) {
        if (this.memoryOnlyMode) {
            this.importStats = stats;
        } else {
            this.setItem('import_stats', JSON.stringify(stats));
        }
    }
    
    static getImportStats() {
        if (this.memoryOnlyMode) {
            return this.importStats || { duplicatesFound: 0, duplicateFieldIds: [] };
        }
        try {
            const data = this.getItem('import_stats');
            return data ? JSON.parse(data) : { duplicatesFound: 0, duplicateFieldIds: [] };
        } catch (error) {
            return { duplicatesFound: 0, duplicateFieldIds: [] };
        }
    }
    
    static importStats = { duplicatesFound: 0, duplicateFieldIds: [] }; // For memory-only mode
